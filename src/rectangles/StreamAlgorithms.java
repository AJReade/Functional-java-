...
public class StreamAlgorithms {
  ...
  public static Stream<Rectangle> translate(Stream<Rectangle> rectangles, Point vector) {
    return rectangles.map(
        rec -> new Rectangle(
            new Point(
                rec.getTopLeft().getX() + vector.getX(),
                rec.getTopLeft().getY() + vector.getY()),
            rec.getWidth(),
            rec.getHeight()
        )
    );
  ...
  public static Stream<Rectangle> getAllIntersecting(
    ...
    return rectangles.filter(rec -> rec.intersects(rectangle));
  }
  ...
  public static int findLargestArea(Stream<Rectangle> rectangles) {
    return rectangles.map(Rectangle::area).reduce(0, (x, y) -> x > y ? x : y);
  }
  ...
  public static int findMaxHeight(Stream<Rectangle> rectangles) {
    return rectangles.map(Rectangle::getHeight).reduce(0, (x, y) -> x > y ? x : y);
  }
